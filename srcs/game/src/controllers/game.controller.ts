...
export async function newGameSession(
  ...
  joinGame.call(this, sessionId, null);
  return {
    status: "success",
...
function joinGame(
  ...
  // Send initial connection confirmation with websocket if socket has been provided
  if (socket) {
    socket.send(
      JSON.stringify({
        type: "newSession",
...
export async function webSocketConnect(
  ...
  handleClientMessage.call(this, socket, game, sessionId, stateInterval);
}
