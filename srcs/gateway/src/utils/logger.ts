...
export interface Logger {
  ...
  logHealth(context: Omit<HealthLogContext, 'event'>, type: 'check' | 'service_check' | 'failed'): void;
}
...
class GatewayLogger implements Logger {
  ...
  logRequest(context: Omit<RequestLogContext, 'event'>, phase: 'start' | 'end' | 'error'): void {
    ...
    const level = phase === 'error' ? 'error' :
                  phase === 'end' && context.status && context.status >= 400 ? 'warn' : 'info';
...
export const createLogContext = {
  ...
  request: (method: string, url: string, user?: string, ip?: string): Omit<RequestLogContext, 'event'> => ({
    method,
    url,
    user: user || null,
    ip: ip || null,
