...
export function registerAuthRoutes(app: FastifyInstance) {
  ...
  app.get("/health", async (request, reply) => {
    ...
    const res = await proxyRequest(app, request, reply, "http://auth-service:3001/health");
  return res;
  });

  app.get("/me", async (request, reply) => {
    app.log.info({ event: 'auth_me', user: request.headers['x-user-name'] || null });
    const res = await proxyRequest(app, request, reply, "http://auth-service:3001/me");
    return res;
  });
  ...
  app.post("/login", async (request, reply) => {
    ...
    const res = await proxyRequest(app, request, reply, "http://auth-service:3001/login", {
      method: "POST",
  ...
  app.post("/register", async (request, reply) => {
    ...
    const body = JSON.stringify(request.body);
    const res = await proxyRequest(app, request, reply, "http://auth-service:3001/register", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body,
    });
  ...
  app.post("/logout", async (request, reply) => {
    ...
    const res = await proxyRequest(app, request, reply, "http://auth-service:3001/logout", {
      method: "POST",
    });
    return res;
  });
